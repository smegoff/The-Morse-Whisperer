/*
  ...
  ARCH UPDATE (v1.2):
   - Two training modes:
       * Manual A10: waits for AAAAAAAAAA, then locks + stops training
       * Auto: locks on stability (dotMs stable + good SNR + enough transitions)
   - Buffered ADC blocks (ring buffer)
   - WDT-safe sampler task (fixes reboot loop)
...
static const char* VERSION_STR  = "v1.2 (2 training modes + buffered sampler + WDT-safe)";
...
// Timing thresholds (in dot units) (kept for compatibility)
...
// ===================== UI State ============================
// Training mode default ON (per request earlier)
static bool trainingMode    = true;
static bool expandShorthand = true;

// Two training modes
enum TrainMode : uint8_t { TRAIN_A10 = 0, TRAIN_AUTO = 1 };
static TrainMode trainMode = TRAIN_A10; // default for competition

// Manual A10 tracker
static const char* TRAIN_EXPECT = "AAAAAAAAAA";
static const uint8_t TRAIN_EXPECT_LEN = 10;
static uint8_t trainPos = 0;
static uint16_t trainErrors = 0;

// Auto-lock stability tracker
static float trainLastDot = 0.0f;
static uint32_t trainStableMs = 0;
static uint32_t trainLastCheckMs = 0;
static uint32_t trainTransitions = 0;
static uint32_t trainStartMs = 0;

// Lock banner
static bool trainingLocked = false;
static uint32_t trainingLockedMs = 0;
...
// ===================== Buffered Audio (Ring Buffer) =========
static const int AUDIO_Q_BLOCKS = 12; // 12 * 20ms = 240ms buffer
static int16_t audioQ[AUDIO_Q_BLOCKS][BLOCK_N];

static volatile uint16_t qWrite = 0;
static volatile uint16_t qRead  = 0;
static volatile uint16_t qCount = 0;
static volatile uint32_t qOverruns = 0;

static portMUX_TYPE qMux = portMUX_INITIALIZER_UNLOCKED;

static inline void qPushBlock(const int16_t* src) {
  portENTER_CRITICAL(&qMux);
  if (qCount >= AUDIO_Q_BLOCKS) {
    qOverruns++;
    qRead = (qRead + 1) % AUDIO_Q_BLOCKS;
    qCount--;
  }
  memcpy(audioQ[qWrite], src, sizeof(int16_t) * BLOCK_N);
  qWrite = (qWrite + 1) % AUDIO_Q_BLOCKS;
  qCount++;
  portEXIT_CRITICAL(&qMux);
}

static inline bool qPopBlock(int16_t* dst) {
  bool ok = false;
  portENTER_CRITICAL(&qMux);
  if (qCount > 0) {
    memcpy(dst, audioQ[qRead], sizeof(int16_t) * BLOCK_N);
    qRead = (qRead + 1) % AUDIO_Q_BLOCKS;
    qCount--;
    ok = true;
  }
...
static void trainingResetState();
static void trainingStopLocked();
static void trainingObserveChar(char c);
static void trainingAutoTick();
...
static const char* trainModeText() {
  return (trainMode == TRAIN_A10) ? "A10" : "AUTO";
}
...
static const char* menuItemLabel(uint8_t idx) {
  switch ((MenuItem)idx) {
    ...
    case MI_TRAINMODE: return "Train Mode";
...
static void oledRenderMain() {
  ...
  String mode = trainingMode ? String("TRAIN(") + trainModeText() + ")" : "RX";
  String l1 = mode +
  ...
  float bar = (snrEma - 1.0f) / 5.0f;
  drawBar(0, 34, 128, 10, bar);

  // brief "LOCKED" banner
  if (trainingLocked && (millis() - trainingLockedMs) < 2500UL) {
    u8g2.drawStr(0, 58, "LOCKED ✅");
  } else {
...
static void oledRenderMenu() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (idx == MI_TRAINMODE) line += String(":") + trainModeText();
...
static const int qrAreaX = 0 + pad;
static const int qrAreaY = 0 + pad;
static const int qrAreaW = 64 - (pad * 2);
static const int qrAreaH = 64 - (pad * 2);

static const int textX = 64 + pad;
static const int textY = 12;
static const int lineH = 12;
...
static void oledQrJoinScreen() {
  ...
  if (ret != ESP_OK) u8g2.drawStr(textX, 63, "QR gen failed");
  ...
  if (qrcodeData) {
    ...
    if (scale < 1) scale = 1;
    if ((qrSize * (scale + 1) <= innerW) && (qrSize * (scale + 1) <= innerH)) scale++;
...
static void adaptDot(float candidate) {
  dotMs = (1.0f - DOT_EMA_ALPHA) * dotMs + DOT_EMA_ALPHA * candidate;
}
...
static void trainingStopLocked() {
  trainingMode = false;
  trainingLocked = true;
  trainingLockedMs = millis();
}

static bool isMeaningfulDecoded(char c) {
  if (c == 0) return false;
  if (c == '#') return true; // meaningful (wrong/unknown)
  if (c == ' ' || c == '\n' || c == '\r' || c == '\t') return false;
  return true;
}

// called whenever we output a decoded character
static void trainingObserveChar(char c) {
  if (!trainingMode) return;

  if (trainMode == TRAIN_A10) {
    // Manual AAAAAAAAAA lock
    if (!isMeaningfulDecoded(c)) return;
    ...
    char expected = TRAIN_EXPECT[trainPos];
    if (c == expected) {
      trainPos++;
      if (trainPos >= TRAIN_EXPECT_LEN) {
        trainingStopLocked();
      }
    } else {
      // Only reset on real symbols (alnum/punct), ignore other noise we already filtered
      trainPos = 0;
      trainErrors++;
    }
  }
...
static void trainingAutoTick() {
  if (!trainingMode) return;
  if (trainMode != TRAIN_AUTO) return;

  uint32_t now = millis();
  if (now - trainLastCheckMs < 200) return; // check ~5Hz

  uint32_t dt = now - trainLastCheckMs;
  trainLastCheckMs = now;

  // Stability heuristic:
  // - dotMs not drifting much
  // - SNR comfortably above squelch
  // - enough tone transitions happened (so we aren't "stable" in silence)
  // - give it a minimum run-up time
  float dd = fabsf(dotMs - trainLastDot);
  trainLastDot = dotMs;

  bool snrGood = (snrEma > (squelchSNR + 0.35f));
  bool dotStable = (dd < 1.0f);                 // ~1ms drift tolerance
  bool enoughTransitions = (trainTransitions >= 18);
  bool minTime = (now - trainStartMs) > 3500UL; // don't insta-lock at boot

  if (snrGood && dotStable) {
    trainStableMs += dt;
  } else {
    trainStableMs = 0;
  }

  if (minTime && enoughTransitions && trainStableMs >= 5000UL) {
    trainingStopLocked();
  }
}
...
static void handleMark(float markMs) {
  ...
  if (markMs >= (DAH_DOT_RATIO_CUTOFF * dotMs)) currentSymbol += "-";
  else                                          currentSymbol += ".";
}
...
// ===================== Sampling (Sampler Task) ===============
// WDT-safe, timing-safe approach.

static void sampleBlockPrecise(int16_t* block) {
  uint32_t t0 = micros();
  const uint32_t usPerSample = (uint32_t)(1000000UL / SAMPLE_RATE);
  ...
  for (int i = 0; i < BLOCK_N; i++) {
    ...
    uint32_t target = t0 + (uint32_t)((i + 1) * usPerSample);

    while ((int32_t)(micros() - target) < 0) {
      uint32_t now = micros();
      uint32_t remain = (target > now) ? (target - now) : 0;
      if (remain > 300) vTaskDelay(1);
      else taskYIELD();
    }

    // if we're behind schedule, still yield occasionally
    if ((i & 0x0F) == 0) taskYIELD();
  }
}

static void audioSamplerTask(void* pv) {
  (void)pv;
  ...
  for (;;) {
    ...
    if (millis() - lastBeat > 5000) {
      lastBeat = millis();
      // Serial.printf("[sampler] q=%u overruns=%lu\n", (unsigned)qCount, (unsigned long)qOverruns);
    }
  }
}
...
// Process ONE queued block if available. If none, yield briefly.
static void processBlock() {
  ...
  if (newTone != tonePresent) {
    noteActivity();
    trainTransitions++; // training stability heuristic uses this
  ...
  if (!tonePresent) {
    ...
    if (silentFor > (uint32_t)(WORD_GAP_UNITS * dotMs * 1.2f)) {
      flushCurrentSymbol();
    }
  }
...
static void calibrateNow() {
  ...
  for (int tries = 0; tries < 14; tries++) {
    if (!qPopBlock(block)) sampleBlockPrecise(block);
  ...
  // reset training counters AFTER calibration
  trainingResetState();
...
<div class="box">
  <div class="row">
    ...
    <div><span class="k">Train:</span> <span id="tmode" class="pill"></span></div>
...
<div class="box">
  <div class="row">
    ...
    <button onclick="fetch('/cycleTrainMode')">Cycle Train Mode</button>
...
static void webBegin() {
  ...
  server.on("/status", [](){
    ...
    json += "\"trainMode\":\"" + String(trainModeText()) + "\",";
    ...
    json += "\"qCount\":" + String((uint16_t)qCount) + ",";
    json += "\"qOverruns\":" + String((uint32_t)qOverruns);
    json += "}";
  ...
  server.on("/cycleTrainMode", [](){
    trainMode = (trainMode == TRAIN_A10) ? TRAIN_AUTO : TRAIN_A10;
    // if training is currently on, reset counters so mode switch is clean
    if (trainingMode) trainingResetState();
    server.send(200, "text/plain", "OK");
  });
...
void setup() {
  ...
  // Start sampler task (pinned to core 1; avoids starving IDLE0)
  ...
  // Auto-calibrate at boot if training mode is ON
  if (trainingMode) calibrateNow();
